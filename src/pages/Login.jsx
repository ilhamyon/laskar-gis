...
function Login() {
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        const sanityData = await sanityClient.fetch(`*[_type == 'user-relawan']{
          _id,
          name,
          email,
          type,
  ...
  const handleLogin = (event) => {
    ...
    const user = serverData.data.find(
      (relawan_userData) => relawan_userData.email === username && relawan_userData.password === password
    );
    ...
    if (user) {
      ...
      localStorage.setItem("relawan_token", token);
      localStorage.setItem("relawan_user", user.name);
      localStorage.setItem("relawan_id", user._id);
      localStorage.setItem('relawan_userData', JSON.stringify(filteredData));
